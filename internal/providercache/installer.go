	...
	for provider, version := range need {
		...
		if i.globalCacheDir != nil {
			...
			if cached := i.globalCacheDir.ProviderVersion(provider, version); cached != nil {
				...
				newHashes := make([]getproviders.Hash, len(preferredHashes), len(preferredHashes)+1)
				copy(newHashes, preferredHashes)
		...
		newHashes := make([]getproviders.Hash, len(preferredHashes), len(preferredHashes)*2+1)
		copy(newHashes, preferredHashes)
